...
- **Alertes coupées en tours de qualification** (demande explicite : "on s'en fout des qualifs") : filtre `/qualif|preliminary|play-?offs?/i` sur `fx.league.round` avant génération du modèle/alertes — se réactive automatiquement dès la phase finale (League Stage / élimination directe). Le match reste visible dans la liste de fixtures (route séparée, non filtrée) ; seuls modèle/snapshot/alertes sont coupés en qualifs.
