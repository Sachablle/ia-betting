...
**MLB — Total runs, mode fantôme (24 juillet 2026, `backend/computeMlb.js` + `backend/server.js` + `src/pages/MlbDetailPage.jsx`) :**
Nouveau sport, scope serré à un seul marché (Over/Under total runs). Décision explicite de l'utilisateur : **validation avant risque réel** — `MLB_ALERTS_ENABLED = false` en dur (`server.js`) tant que la calibration n'est pas vérifiée sur les données réelles accumulées ; le modèle tourne à chaque cycle et log dans le near-miss (voir ci-dessous) mais ne pousse jamais de vraie alerte (`newAlerts.push` jamais atteint).
- **Sources** : MLB Stats API (`statsapi.mlb.com`, gratuite, sans clé) pour matchs + scores + effectifs — `schedule` couvre les deux en un seul endpoint. Piège : `stats?stats=lastXGames&numGames=N` ignore le paramètre et renvoie la saison entière (vérifié en direct) — la forme récente est reconstruite depuis `/schedule` filtré par équipe+dates. Cotes Total Runs : Betclic (HTML direct, pas de gRPC) + Unibet (`groupedMarkets`, même format que le foot) ; rapprochement des noms par surnom seul (`mlbTeamMatches`/`mlbNicknameSuffix`) plutôt qu'une table d'alias à 30 entrées, sauf exception "Wh.Sox" (White Sox) ajoutée après test réel. Logos : CDN ESPN (`a.espncdn.com/i/teamlogos/mlb/500/{abbr}.png`) — le CDN RotoWire bloque le hotlinking par vérification du Referer (403 hors rotowire.com), invisible en `curl` simple qui n'envoie pas de Referer par défaut. Seule exception sur les 30 franchises : Arizona (`az` chez MLB Stats API, `ari` chez ESPN).
- **Modèle** (`computeMlb.js`) : Poisson, même principe que `computeFootball.js` mais sans grille jointe Dixon-Coles (un seul marché) — `λtotal = λhome + λaway` suffit. `computeMlbLambdas()`, `MLB_HOME_ADV = 1.03` (repère littérature, non calibré), shrinkage petit échantillon dupliqué (sports indépendants, pas importé de `computeFootball.js`). λ league moyen calculé dynamiquement sur le pool d'équipes qui jouent le jour même (même principe que la CDM). **Limite connue, pas encore corrigée** : ni split domicile/extérieur ni park factor — les stats "extérieur" d'une équipe qui joue souvent à Coors Field (Rockies, stade en altitude très favorable à l'attaque) sont polluées par ses matchs à domicile ; raffinement candidat, en attente des données du mode fantôme avant de décider si nécessaire.
- **Routes** : `GET /api/mlb/preview` (fusion fixtures + cotes + probabilités par ligne, un seul appel — évite le double scraping qu'un premier essai avait avec `/odds` + `/preview` séparés) ; `GET /api/mlb/lineups` (RotoWire, `rotowire.com/baseball/daily-lineups.php`, même famille HTML `lineup__*` que le basket mais adaptée : `lineup__pos` = position défensive, ordre des `<li>` = ordre de batte réel, lanceur partant séparé via `lineup__player-highlight` ; statut `Confirmé`/`Probable` extrait en direct de la classe CSS `is-confirmed`/`is-expected`, vert `#22c55e` au tip-off comme NBA/WNBA) ; `GET /api/mlb/teams` / `GET /api/mlb/roster/:teamId` (Base de données, `EffectifPage.jsx`, cache 6h, **volontairement sans colonnes de stats** — pas de split hitting/pitching unifié pertinent pour un roster mixte lanceurs/joueurs de champ).
- **Frontend** : visible dans la Carte du Monde sous États-Unis via une icône ⚾ dédiée (`WorldMapPage.jsx`, `sportOf(l)` généralisé de binaire foot/basket à 3 catégories — n'apparaît que pour les pays ayant effectivement une ligue baseball, contrairement à foot/basket toujours affichés en duo, `_hasBaseball`). Pour les États-Unis spécifiquement (seul pays avec MLB mais sans foot), l'icône ⚽ désactivée est retirée plutôt que grisée — tout autre pays basket-only sans MLB garderait le ⚽ grisé comme avant (aucun cas aujourd'hui). `MlbDetailPage.jsx` homogénéisée sur le style foot/basket (hero bilan V-D + forme, cotes repliables, panneau compositions RotoWire en lecture seule — `LineupBuilder`/`RosterPanel` du basket volontairement pas réutilisés, trop couplés à l'édition de compo props qui n'existe pas en MLB) avec bandeau explicite "sport en phase de test" pour ne jamais laisser croire que l'estimation affichée est une alerte. Tableau Total runs : sélecteur "Ligne ⌄" (badge cliquable + popup, réutilise le pattern popup absolute + click-outside déjà présent dans `MatchDetailPage.jsx`/`BasketballDetailPage.jsx` pour l'écart foot/basket) au lieu d'une grille large toutes-lignes ; ligne par défaut = médiane des lignes dispo ; en-têtes de colonnes Over/Under affichent la ligne elle-même, % du modèle affiché à part en bas à droite.
- **How to apply — étape suivante (pas encore faite)** : après quelques semaines de données réelles dans `near_miss_mlb.json`, comparer `probability` vs `status` (won/lost) par tranche de probabilité. Si bien calibré → `MLB_ALERTS_ENABLED = true` + brancher un vrai bloc `newAlerts.push` (calquer sur `football_total`). Si mauvais → raffiner (split domicile/extérieur, park factor) ou abandonner le sport comme convenu avec l'utilisateur. **Ne jamais activer `MLB_ALERTS_ENABLED` sans avoir vérifié cette calibration d'abord.**
