...
7 compétitions (Ligue 1, PL, La Liga, Serie A, Bundesliga, NBA, WNBA), 3 sources bookmaker fusionnées car aucune ne couvre tout de façon fiable seule — toutes scrapées via Playwright (le marché "Vainqueur Compétition" n'est pas dans le HTML brut, contrairement aux marchés match-par-match déjà scrapés par `fetchBk`) :
...
- **Pinnacle** (`fetchPinnacleOutrights()`) — NBA uniquement (seul marché outright trouvé chez eux en explorant), `_scrapePinnacleNBAOutright()`.
